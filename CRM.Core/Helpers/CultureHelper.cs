...
namespace CRM.Core.Helpers
{
    public class CultureHelper
    {
        ...
        internal static void GetCultureName(object p)
        {
        ...
        "ar",  // first culture is the DEFAULT
        "en"  
        ...
        {
            return Cultures[1]; // return Default culture
        }
        ...
        public static string GetCultureName(HttpCookie cultureCookie)
        {
            var cultureName = cultureCookie != null ? cultureCookie.Value : Constants.LanguageCultures.Arabic;
            // Validate culture name
            cultureName = CultureHelper.GetImplementedCulture(cultureName); // This is safe
            return cultureName;
        }

        public static int MapLanguage(string language)
        {
            switch (language)
            {
                case Constants.LanguageCultures.Arabic:
                    return (int)GeneralEnums.LanguageEnum.Arabic;
                case Constants.LanguageCultures.Hebrew:
                    return (int)GeneralEnums.LanguageEnum.English;
                default:
                    return (int)GeneralEnums.LanguageEnum.Arabic;
            }
        }

        public static int GetCurrentLanguageId(HttpCookie cultureCookie)
        {
            var cultureName = cultureCookie != null ? cultureCookie.Value : Constants.LanguageCultures.Arabic;
            // Validate culture name
            cultureName = CultureHelper.GetImplementedCulture(cultureName); // This is safe
            return MapLanguage(cultureName);
        }
