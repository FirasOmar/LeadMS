...
namespace CRM.Web.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var userProfile = _db.UserProfiles.FirstOrDefault(r => r.Username == model.Email);

            if (userProfile == null)
            {
                ModelState.AddModelError("", App_GlobalResources.Account.Login.ErrorInvalidLoginAttempt);
                return View(model);
            }

            if (userProfile.Status == (int)GeneralEnums.StatusEnum.Deactive)
            {
                ModelState.AddModelError("", App_GlobalResources.Account.Login.TxtAccountDeActivated);
                return View(model);
            }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var account = _db.UserProfiles.FirstOrDefault(r => r.Username == model.Email && r.Status != (int)GeneralEnums.StatusEnum.Deleted);
                if (account != null && account.Status == (int)GeneralEnums.StatusEnum.Deactive)
                {

                    ModelState.AddModelError("", "البريد الالكتروني غير فعّال , يرجى مراجعه المسؤول لتفعيل حسابك");
                }
                else
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        try
                        {
                            var userProfile = new UserProfile
                            {
                                ...
                                GenderLookupId = model.GenderLookupId,
                                IdNumber = model.IdNumber ?? string.Empty,
                                MartialStatusLookupId = model.MartialStatusLookupId,
                            ...
                            var userProfileTrans = new UserProfileTranslation()
                            {
                                ...
                                IsDefault = languageId == (int)GeneralEnums.LanguageEnum.Arabic,
                                LanguageId = languageId,
                                UserProfileId = userProfile.Id
                            };

                            _db.UserProfileTranslations.Add(userProfileTrans);
                            if (!userProfileTrans.IsDefault)
                            {
                                var userProfileTrans1 = new UserProfileTranslation()
                                {
                                    FullName = model.FullName,
                                    Address = model.Address ?? string.Empty,
                                    IsDefault = true,
                        ...
                        {
                            LogHelper.LogException(User.Identity.Name, ex, "Error While register new user!");
            ...
            ViewBag.GenderLookupId = new SelectList(LookupHelper.GetLookupDetailsByMasterId((int)GeneralEnums.MasterLookupEnums.Gender, Request.Cookies["_culture"]), "Id", "Name", model.GenderLookupId);
            //ViewBag.MartialStatusLookupId = new SelectList(LookupHelper.GetLookupDetailsByMasterId((int)GeneralEnums.MasterLookupEnums.MartialStatus, Request.Cookies["_culture"]), "Id", "Name", model.MartialStatusLookupId);
            //ViewBag.ProvinceId = new SelectList(LookupHelper.GetLookupDetailsByMasterId((int)GeneralEnums.MasterLookupEnums.Province, Request.Cookies["_culture"]), "Id", "Name", model.ProvinceId);
            //ViewBag.RegionId = new SelectList(LookupHelper.GetLookupDetailsByMasterId((int)GeneralEnums.MasterLookupEnums.Region, Request.Cookies["_culture"]), "Id", "Name", model.RegionId);
            //ViewBag.CityId = new SelectList(LookupHelper.GetLookupDetailsByMasterId((int)GeneralEnums.MasterLookupEnums.City, Request.Cookies["_culture"]), "Id", "Name", model.CityId);
            var prefferdLanguage = LookupHelper.GetLookupDetailsByMasterId((int)GeneralEnums.MasterLookupEnums.PrefferdLanguage,
                Request.Cookies["_culture"]);
